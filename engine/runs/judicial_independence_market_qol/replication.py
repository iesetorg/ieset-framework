#!/usr/bin/env python3
"""Replication — Judicial independence and market QoL.

Spec: Stronger rule-of-law proxies strengthen GDP per capita outcomes.

Design:
  1. Panel 1996-2019 (WGI coverage).
  2. WDI NY.GDP.PCAP.KD as outcome.
...
HID = "judicial_independence_market_qol"
...
def main() -> None:
    ...
    df = gdppc.merge(rl, on=["country", "year"], how="outer")
    ...
    df = df.dropna(subset=["log_gdppc", "rl", "ge", "trade"])
    if len(df) < 50:
    ...
    exog = df[["rl", "ge", "trade"]]
    model = PanelOLS(df["log_gdppc"], exog, entity_effects=True, time_effects=True)
    ...
    if beta > 0 and pval < 0.10:
        verdict = f"supported — β_RL={beta:+.4f} (SE {se:.4f}, p={pval:.3f}, n={n}). Stronger rule of law predicts higher GDP pc."
    ...
    (OUT_DIR / "result_card.md").write_text(f"# Result card — {HID}\n\n**Verdict:** {verdict}\n\nPanel FE with country and year fixed effects, log GDP pc as outcome.\n")
