#!/usr/bin/env python3
"""Run-local replication for eurostat_energy_price_export_competitiveness_panel."""
...
HYPOTHESIS_ID = "eurostat_energy_price_export_competitiveness_panel"
...
CLAIM = "Higher industrial electricity prices reduce export competitiveness."
SCHOOLS = ("classical_liberal", "developmentalism", "ordoliberal")
...
def main() -> int:
    ...
    exports, export_meta = wdi("NE.EXP.GNFS.ZS", "export_share")
    gdp, gdp_meta = wdi("NY.GDP.PCAP.KD.ZG", "gdp_pc_growth")
    panel = price.merge(exports, on=["country", "year"], how="outer").merge(gdp, on=["country", "year"], how="outer")
    keys = ["industrial_electricity_price", "export_share", "gdp_pc_growth"]
    ...
    formula = "Q('export_share') ~ Q('industrial_electricity_price') + Q('gdp_pc_growth') + C(country) + C(year)"
    ...
    chart = d[["country", "year", "industrial_electricity_price", "export_share"]].sort_values(["country", "year"]).to_dict(orient="records")
