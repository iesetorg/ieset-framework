...
**Verdict:** INCONCLUSIVE_DATA_PENDING — outcome 'resident_deposit_usd_share' not loaded; missing: ['imf:DCFD_USD_DC; world_bank_wdi:FB.AST.NPER.ZS', 'imf:FIDR; world_bank_wdi:FR.INR.DPST']
...
## Estimate
- _Error:_ outcome 'resident_deposit_usd_share' not loaded; missing: ['imf:DCFD_USD_DC; world_bank_wdi:FB.AST.NPER.ZS', 'imf:FIDR; world_bank_wdi:FR.INR.DPST']

## Variables resolved
- `bis:WS_EER` → real_effective_exchange_rate (outcome, publisher=bis, n=2922)
- `world_bank_wdi:BX.TRF.PWKR.DT.GD.ZS` → remittance_share_of_gdp_change (treatment, publisher=world_bank_wdi, n=8915)
- `imf:PCPIPCH` → cpi_inflation (controls, publisher=imf, n=10789)
- `fred:DCOILBRENTEU; imf_pcps:POILBRE` → oil_price (controls, publisher=fred, n=40)

### Variables missing data
- `imf:DCFD_USD_DC; world_bank_wdi:FB.AST.NPER.ZS` (outcome, name=resident_deposit_usd_share) — vintage not on disk
- `imf:FIDR; world_bank_wdi:FR.INR.DPST` (outcome, name=real_local_currency_deposit_rate) — vintage not on disk
...
- `fred:USREC` (controls, name=us_recession_indicator) — vintage not on disk
