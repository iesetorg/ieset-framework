#!/usr/bin/env python3
"""Replication — IP protection and innovation diffusion.

Spec: Stronger rule-of-law proxies predict stronger high-technology exports.

Design:
  1. Panel 1996-2019 (WGI coverage).
  2. WDI TX.VAL.TECH.MF.ZS (high-tech exports % manufactured exports) as outcome.
...
HID = "ip_protection_innovation_diffusion"
...
def main() -> None:
    ...
    tech_path = latest("world_bank_wdi", "TX.VAL.TECH.MF.ZS")
    rl_path = latest("wgi", "GOV_WGI_RL.EST")
    gdppc_path = latest("world_bank_wdi", "NY.GDP.PCAP.KD")
    ge_path = latest("wgi", "GOV_WGI_GE.EST")
    manifest = {
        "wdi_tech": {"publisher": "world_bank_wdi", "series": "TX.VAL.TECH.MF.ZS", "vintage_file": str(tech_path.relative_to(REPO_ROOT)), "sha256": sha256(tech_path)},
    ...
    df = df.dropna(subset=["tech", "rl", "log_gdppc", "ge"])
    if len(df) < 50:
    ...
    model = PanelOLS(df["tech"], exog, entity_effects=True, time_effects=True)
    ...
    if beta > 0 and pval < 0.10:
        verdict = f"supported — β_RL={beta:+.4f} (SE {se:.4f}, p={pval:.3f}, n={n}). Stronger rule of law predicts higher tech exports."
