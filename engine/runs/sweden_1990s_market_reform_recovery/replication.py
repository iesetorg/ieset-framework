#!/usr/bin/env python3
"""Replication — Sweden 1990s market reform and recovery, 1991-2019.

Spec: Sweden's post-crisis market reforms and fiscal rules predict stronger 1995-2019
growth than its pre-crisis state-expansion model.

Design:
  1. Sweden + Nordic/EU comparator set.
  2. PWT RGDPE per capita 1991-2019.
  3. Compare Sweden's convergence slope to Nordic/EU median.

Falsification:
  SUPPORTED if Sweden slope >= comparator median + 0.30 pp/yr.
  REFUTED if Sweden slope <= comparator median.
  Otherwise PARTIAL.
"""
...
HID = "sweden_1990s_market_reform_recovery"
...
TARGET = "SWE"
FRONTIER = "DEU"
COMPARATORS = ["DNK", "NOR", "FIN", "NLD", "AUT", "BEL", "CHE"]
...
def main() -> None:
    ...
    for iso3 in countries:
        ...
        rows.append({"country_iso3": iso3, "convergence_slope": slope, "is_sweden": iso3 == TARGET})
    ...
    swe = results[results["is_sweden"]]
    comp = results[~results["is_sweden"]]
    swe_slope = float(swe["convergence_slope"].iloc[0]) if len(swe) else float("nan")
    comp_median = float(comp["convergence_slope"].median())
    diff = swe_slope - comp_median
    if swe_slope >= comp_median + DIFF_THRESHOLD:
        verdict = f"supported — Sweden slope {swe_slope*100:+.2f}pp/yr vs median {comp_median*100:+.2f}pp/yr (diff {diff*100:+.2f}pp)."
    elif swe_slope <= comp_median:
        verdict = f"refuted — Sweden slope {swe_slope*100:+.2f}pp/yr at or below median {comp_median*100:+.2f}pp/yr."
    else:
        verdict = f"partial — Sweden slope {swe_slope*100:+.2f}pp/yr vs median {comp_median*100:+.2f}pp/yr (diff {diff*100:+.2f}pp)."
    diagnostics = {
        "hypothesis_id": HID, "verdict": verdict.split(" — ")[0],
        "reason": verdict.split(" — ")[1] if " — " in verdict else verdict,
        "metrics": {"n_comparators": int(len(comp)), "sweden_slope": swe_slope, "comparator_median": comp_median, "diff": diff},
    ...
    (OUT_DIR / "result_card.md").write_text(f"# Result card — {HID}\n\n**Verdict:** {verdict}\n\nSweden vs {len(comp)} Nordic/EU comparators, endpoint slope {BASE_YEAR}-{END_YEAR}.\n")
