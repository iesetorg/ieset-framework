#!/usr/bin/env python3
"""Replication — Discretionary state allocation and corruption.

Spec: Higher discretionary state-allocation burden (government consumption share)
predicts lower control-of-corruption scores across a broad panel.

Design:
  1. Panel 1996-2019 (WGI coverage).
  2. WGI CC.EST as outcome.
  3. WDI NE.CON.GOVT.ZS as treatment.
  4. Controls: log GDP pc, WGI RL.EST.
  5. PanelOLS with country and year FE, clustered SEs.

Falsification:
  SUPPORTED if β_gov_consumption < 0 and p < 0.10.
  REFUTED if β_gov_consumption > 0 and p < 0.10.
  Otherwise PARTIAL.
"""
...
HID = "discretionary_allocation_corruption_panel"
...
def main() -> None:
    ...
    df = cc.merge(gc, on=["country", "year"], how="outer")
    ...
    df = df.dropna(subset=["cc", "gc", "log_gdppc"])
    if len(df) < 50:
    ...
    if beta < 0 and pval < 0.10:
        verdict = f"supported — β_gc={beta:+.4f} (SE {se:.4f}, p={pval:.3f}, n={n}). Higher gov consumption predicts lower corruption control."
