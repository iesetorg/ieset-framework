...
## Variables resolved
- `shiller:ie_data (USA); bis:WS_SPP (all)` → cumulative_real_equity_return (outcome, n=153)
- `oecd:OECD.SDD.TPS,DSD_PRICES@DF_PRICES_N_CP,1.0; fred:CPILFESL (USA fallback)` → cumulative_core_cpi (outcome, n=70)
- `fred:WALCL; fred:GDP; ecb:ILM; boe:BalanceSheet; boj:monetary_base; fred:JPNASSETS` → central_bank_balance_sheet_to_gdp (treatment, n=25)
- `fred:DFF (USA); ecb:FM (EUR); boe:IUDSOIA (GBR); boj:policy_rate (JPN)` → short_rate (controls, n=73)
