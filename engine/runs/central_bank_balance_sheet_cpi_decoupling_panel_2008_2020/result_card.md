...
## Variables resolved
- `fred:CPILFESL (USA); boj:core_cpi (JPN); ecb:ICP_core (EUR); ons:cpi_core (GBR)` → core_cpi_inflation_yoy (outcome, publisher=fred, n=70)
- `fred:M2SL (USA); ecb:BSI.M.U2.M2 (EUR); boj:money_stock_m2; boe:LPMVQJW (GBR)` → m2_broad_money_growth (outcome, publisher=fred, n=68)
- `fred:WALCL; fred:GDP (USA); boj:monetary_base; fred:JPNNGDP (JPN); ecb:ILM; ecb:NGDP (EUR); boe:BalanceSheet; boe:GDP (GBR)` → central_bank_balance_sheet_pct_gdp (treatment, publisher=fred, n=25)
- `fred:BOGMBASE (USA); boj:monetary_base; ecb:base_money; boe:monetary_base` → monetary_base_growth (treatment, publisher=fred, n=68)
- `fred:DFF (USA); ecb:FM (EUR); boj:policy_rate (JPN); boe:bank_rate (GBR)` → short_rate (controls, publisher=fred, n=73)
- `fred:UNRATE; ilostat:unemployment_rate` → unemployment_rate (controls, publisher=fred, n=79)
...
- `fred:T5YIFR (USA); ecb:5Y5Y_inflation_swap (EUR); boe:gilt_implied_inflation_5y5y (GBR); boj:5y5y_inflation_swap (JPN)` (outcome, name=inflation_expectations_5y5y_forward) — vintage not on disk
- `imf:NGAP_NPGDP; oecd:OutputGap` (controls, name=output_gap) — vintage not on disk
- `imf_pcps:POILBRE` (controls, name=oil_price) — vintage not on disk
