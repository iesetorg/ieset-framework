...
def main():
    ...
    all_pass = sign_2008_ok and sig_2008 and sign_brx_ok and sig_brx and sc_ok

    if all_pass:
        verdict = "SUPPORTED — UK declined relative to anglophone/advanced donor pool post-2008, deepened post-2016"
    else:
        parts = []
        if not (sign_2008_ok and sig_2008): parts.append(f"post-2008 β={b2008.get('estimate', 0):+.3f} p={b2008.get('p', float('nan')):.3f}")
        if not (sign_brx_ok and sig_brx): parts.append(f"post-brexit β={bbrx.get('estimate', 0):+.3f} p={bbrx.get('p', float('nan')):.3f}")
        if not sc_ok: parts.append(f"SC post-2016 frac-neg={sc_2016_frac:.2f}")
        verdict = "partial — " + "; ".join(parts)
