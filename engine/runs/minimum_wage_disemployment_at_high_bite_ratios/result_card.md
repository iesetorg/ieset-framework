...
Promotion verdict: inconclusive (method-validity gate fails on data availability — the primary state-level BLS outcome and median-wage bite-ratio inputs are not on disk; OECD contextual panels are present but cannot replace the preregistered US state cohort design). Per research documentation a data gap is NOT a refutation — the scoreboard treats this as neutral. Re-run when (a) the BLS state fan-out lands (LAU state teen E/P panel, OES state median hourly wage), and (b) sibling-panel signs reconcile with the preregistered high-bite disemployment direction.
