  ...
  if (existsSync(registrationIndexPath)) {
    try {
      ...
      for (const row of Object.values(index.registrations ?? {})) {
        // Store the full git SHA so the site can deep-link to the real
        // pre-registration commit (not HEAD). UI layers may shorten for display.
        map.set(row.path, {
          hash: row.spec_commit,
  ...
  try {
    ...
    for (const line of out.split("\n")) {
      if (line.startsWith("COMMIT ")) {
        ...
        if (!map.has(line)) {
          map.set(line, { hash: curHash, iso: curIso });
        }
      }
    }
  } catch {
...
export interface RunArtifacts {
  ...
  run_dir_rel?: string;
  /** Wall-clock generation time from result_card.md or diagnostics, if present. */
  generated_at?: string;
  /** First git commit that introduced any artifact under the run directory. */
  run_first_commit?: { hash: string; iso: string } | null;
}
...
export async function loadRunArtifacts(hypothesisId: string): Promise<RunArtifacts> {
  ...
  const promise = (async (): Promise<RunArtifacts> => {
    ...
    if (existsSync(cardPath)) {
      ...
      const genMatch = raw.match(
        /_Generated by\s+`?[^`\n]+`?\s+at\s+([0-9T:+\-Z.]+)_/i
      );
      if (genMatch?.[1]) out.generated_at = genMatch[1];
      const processed = await remark().use(remarkHtml).process(raw);
      out.result_card_html = processed.toString();
    }

    // Prefer diagnostics timestamp when card footer is missing.
    if (!out.generated_at && out.diagnostics) {
      const d = out.diagnostics;
      const cand =
        (d.generated_at as string | undefined) ||
        (d.generated_utc as string | undefined) ||
        (d.run_utc as string | undefined);
      if (cand) out.generated_at = cand;
    }

    // First run commit from the preregistration index (when available).
    try {
      const indexPath = join(REPO_ROOT, "engine", "preregistration_index.json");
      if (existsSync(indexPath)) {
        const index = JSON.parse(readFileSync(indexPath, "utf8")) as {
          registrations?: Record<
            string,
            {
              run_first_commit?: string;
              run_committed_at?: string;
            }
          >;
        };
