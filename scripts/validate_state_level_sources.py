#!/usr/bin/env python3
"""Validate the state-level source inventory and ingestion queue."""
...
INVENTORY_PATH = ROOT / "data" / "state_level" / "source_inventory.yaml"
QUEUE_PATH = ROOT / "data" / "state_level" / "ingestion_queue.yaml"
SECOND_ORDER_SOURCES_PATH = ROOT / "data" / "second_order_sources.yaml"
INVENTORY_SCHEMA = ROOT / "schemas" / "state_level_source_inventory.schema.json"
QUEUE_SCHEMA = ROOT / "schemas" / "state_level_ingestion_queue.schema.json"
...
def cross_reference_errors(
    ...
    state_universe = inventory.get("canonical_state_universe") or {}
    anchors = [state_universe.get("preferred_anchor")]
    anchors.extend(state_universe.get("fallback_anchors") or [])
    for anchor in [a for a in anchors if a]:
        if anchor not in known_sources:
            errors.append(
                f"{INVENTORY_PATH.relative_to(ROOT)}: canonical_state_universe references "
