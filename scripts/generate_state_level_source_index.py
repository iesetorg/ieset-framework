#!/usr/bin/env python3
"""Generate a state-level source index from the inventory and ingestion queue."""
...
import validate_state_level_sources as validator


ROOT = Path(__file__).resolve().parents[1]
OUT_JSON = ROOT / "engine" / "state_level_source_index.json"
OUT_MD = ROOT / "engine" / "state_level_source_index.md"


def load_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    errors = validator.validate()
    if errors:
        raise ValueError("state-level source validation failed:\n" + "\n".join(errors))
...
def compact_source(rec: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "policy_domains": rec["policy_domains"],
...
def compact_wave(wave: dict[str, Any], source_by_id: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for sid in source_ids:
        domains.update(source_by_id[sid]["policy_domains"])
    return {
        ...
        "policy_domain_counts": sorted_counts(domains),
    }
...
def build_index() -> dict[str, Any]:
    ...
    for rec in records:
        ...
        by_scalability[rec["admin1_scalability"]] += 1
        by_geography[rec["geography"]] += 1
        by_domain.update(rec["policy_domains"])
    ...
    return {
        "generated_by": "scripts/generate_state_level_source_index.py",
        "summary": {
            "source_count": len(records),
            "wave_count": len(waves),
            "primary_scope": inventory["primary_scope"],
            "preferred_state_anchor": inventory["canonical_state_universe"]["preferred_anchor"],
            "verification_status_counts": sorted_counts(by_status),
            "ingestion_difficulty_counts": sorted_counts(by_difficulty),
            "admin1_scalability_counts": sorted_counts(by_scalability),
            "policy_domain_counts": sorted_counts(by_domain),
        ...
        "canonical_state_universe": inventory["canonical_state_universe"],
...
def write_outputs(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Preferred state anchor: `{summary['preferred_state_anchor']}`",
        ...
        f"- Admin1 scalability: `{json.dumps(summary['admin1_scalability_counts'], sort_keys=True)}`",
        "",
        "## Policy Domains",
        "",
    ]
    for domain, count in list(summary["policy_domain_counts"].items())[:15]:
    ...
    for rec in payload["top_ingestion_candidates"][:30]:
        use = rec["state_level_use"].replace("|", "/")
        lines.append(
