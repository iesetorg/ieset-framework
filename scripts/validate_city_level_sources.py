#!/usr/bin/env python3
"""Validate the city-level source inventory and ingestion queue.

This is narrower than scripts/validate_specs.py on purpose: the city-level
catalog is a research-to-ingestion planning surface, not a hypothesis spec. The
checks here keep append-heavy scout output honest:

* source_inventory.yaml and ingestion_queue.yaml conform to schemas;
* source IDs are unique;
* city-universe anchors resolve to inventory records;
* every queue source_id resolves to the inventory; and
* every queue source_family_id resolves to data/second_order_sources.yaml.
"""
...
INVENTORY_PATH = ROOT / "data" / "city_level" / "source_inventory.yaml"
QUEUE_PATH = ROOT / "data" / "city_level" / "ingestion_queue.yaml"
SECOND_ORDER_SOURCES_PATH = ROOT / "data" / "second_order_sources.yaml"
INVENTORY_SCHEMA = ROOT / "schemas" / "city_level_source_inventory.schema.json"
QUEUE_SCHEMA = ROOT / "schemas" / "city_level_ingestion_queue.schema.json"
...
def source_ids(inventory: dict[str, Any]) -> list[str]:
    return [rec.get("source_id") for rec in inventory.get("source_records", [])]
...
def cross_reference_errors(
    ...
    city_universe = inventory.get("canonical_city_universe") or {}
    anchors = [city_universe.get("preferred_anchor")]
    anchors.extend(city_universe.get("fallback_anchors") or [])
    for anchor in [a for a in anchors if a]:
        if anchor not in known_sources:
            errors.append(
                f"{INVENTORY_PATH.relative_to(ROOT)}: canonical_city_universe references "
                f"unknown source_id {anchor!r}"
            )
